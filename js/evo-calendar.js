...
(function (factory) {
            ...
            _.initials = {
                ...
                dates: {
                    en: {
                        ...
                        noEventForToday: "No classes today, take a rest! :)",
                        noEventForThisDay: "No classes today, take a rest! :)",
    ...
    EvoCalendar.prototype.addEventList = function (event_data) {
        ...
        markup += '<div class="event-icon"><div id="dei' + (event_data.id) + '" class="event-bullet-' + event_data.type + '"';
        if (event_data.color) {
        ...
        markup += '</div>';
        if (event_data.type == 'deadline')
            markup += '<div><input id="' + event_data.id + '" type="checkbox" onchange="handover(' + event_data.id + ');"></div>';

        markup += '</div>';

        markup += 
        '<script>' +
            'function handover(lessonId) {' +
                'var xhr = new XMLHttpRequest();' +
                'xhr.open("POST", SERVER_URL + "/student/handover", true);' +
                'xhr.setRequestHeader("Content-Type", "application/json");' +
                'xhr.setRequestHeader("accessToken", localStorage.getItem("accessToken"));' +
                'xhr.setRequestHeader("lessonId", lessonId);' +
                'xhr.send();' +
                'xhr.onload = function() {' +
                    //'alert("Status changed");' +
                    'document.getElementById(lessonId).setAttribute("type", "hidden");' +
                    'document.getElementById("dei" + lessonId).setAttribute("style", "background-color: #77fd77");' +
                    //'document.getElementById(id="dei + ' + (event_data.id) + '").style = "background-color: #000111"'; +
                    //'location.reload()' +
                '};' + 
            '}' +
        '</script>'
    ...
    function changeStatus(id) {
        var element = _.$elements.eventEl.find('[data-event-index="' + (id) + '"]')
            .off('click.evocalendar')
            .on('click.evocalendar', _.selectEvent);

            alert('Status changed to: ' + element.value);
    ...
    EvoCalendar.prototype.addEventIndicator = function (event) {
        ...
        function appendDot(date) {
            ...
            if (thisDate.find('span.event-indicator > .type-bullet > .type-' + type).length === 0) {
                ...
                if (event.color) {
                    htmlToAppend += 'id="dot' + event.id + '" style="background-color:' + event.color + '"'
                }
